...
function load_and_prepare_data()
    ...
    # Load CSV data
    df = CSV.read("simulation_results/RI_simulation_results.csv", DataFrame)
...
# ============================================================================
# STATISTICAL HYPOTHESIS TESTING: Optimal vs H1
# ============================================================================
...
function perform_hypothesis_tests(df)
    ...
    heuristic_1_rewards = df.heuristic_1_reward

    # Paired t-test for optimal vs nearest hospital
    diffs = optimal_rewards .- heuristic_1_rewards
    n = length(diffs)
    ...
    println("Paired t-test: Optimal vs H1 Policy")
...
function analyze_extreme_cases(df)
    ...
    improvement = df.optimal_action_reward .- df.heuristic_1_reward
...
function print_cases_optimal_vs_h1_diff(df; save_csv::Bool=true)
    ...
    # Among those, find where reward is (almost) identical
    ...
    if n_same > 0
        ...
        println("  → No cases where rewards are identical but hospitals routed to differ.")
    end
    ...
    println("\nPatient counts by hospital (Optimal Policy):")
    for h in unique_opt_hospitals
        n_routed = count(==(h), df.optimal_action)
        println("  $h: $n_routed patients")
    end

    println("\nPatient counts by hospital (Heuristic 1):")
    for h in unique_h1_hospitals
        n_routed = count(==(h), df.heuristic_1_action)
        println("  $h: $n_routed patients")
    end

    # Save the filtered divergent DataFrame
    if save_csv && n_diff > 0
    ...
    elseif save_csv
        println("No divergent cases to save.")
    end
end
...
function create_histogram_comparison(df)
    ...
    histogram(nearest, bins=40, alpha=0.35, color=darker_gray, label="Nearest CSC",
    ...
    savefig("simulation_results/RI_optimal_vs_nearestCSC_histogram.pdf")
    println("✓ Saved histogram to simulation_results/RI_optimal_vs_nearestCSC_histogram.pdf")
end
